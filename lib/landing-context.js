 ...
 *   user:             Current authenticated user, or null. Used by
 *                     partials/nav.ejs to show appropriate links.
 *
...
async function buildLandingContext(req) {
  const slug = process.env.POLSIA_ANALYTICS_SLUG || '';

  let user = null;
  const cookieStr = req?.headers?.cookie;
  if (cookieStr) {
    const match = cookieStr.split(';').find(c => c.trim().startsWith('session_id='));
    if (match) {
      const sessionId = match.split('=')[1].trim();
      if (sessionId) {
        // Lazy-import pool to avoid circular require at module load time
